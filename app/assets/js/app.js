...
let deviceScannerStream=null,deviceScannerFrame=0,deviceQrDetector=null,deviceScannerBusy=false,deviceScannerLastScan=0,deviceScannerCanvas=null;
...
function stopDeviceScanner(){cancelAnimationFrame(deviceScannerFrame);deviceScannerFrame=0;deviceScannerBusy=false;deviceScannerLastScan=0;deviceScannerStream?.getTracks().forEach(track=>track.stop());deviceScannerStream=null;const video=document.getElementById('device-scanner-video');if(video){video.pause();video.srcObject=null;}}
function closeDeviceScanner(){stopDeviceScanner();document.getElementById('modal-device-scanner')?.classList.remove('on');}
async function scanDeviceQrFrame(now=0){const modal=document.getElementById('modal-device-scanner'),video=document.getElementById('device-scanner-video');if(!modal?.classList.contains('on')||!deviceScannerStream)return stopDeviceScanner();if(!deviceScannerBusy&&video?.readyState>=2&&now-deviceScannerLastScan>140){deviceScannerBusy=true;deviceScannerLastScan=now;try{let raw='';if(deviceQrDetector){const codes=await deviceQrDetector.detect(video);raw=codes[0]?.rawValue||'';}else if(typeof jsQR==='function'){deviceScannerCanvas=deviceScannerCanvas||document.createElement('canvas');const max=720,scale=Math.min(1,max/video.videoWidth),width=Math.max(1,Math.round(video.videoWidth*scale)),height=Math.max(1,Math.round(video.videoHeight*scale)),ctx=deviceScannerCanvas.getContext('2d',{willReadFrequently:true});deviceScannerCanvas.width=width;deviceScannerCanvas.height=height;ctx.drawImage(video,0,0,width,height);raw=jsQR(ctx.getImageData(0,0,width,height).data,width,height,{inversionAttempts:'attemptBoth'})?.data||'';}const token=cloudInviteTokenFromQr(raw);if(token){stopDeviceScanner();modal.classList.remove('on');await acceptCloudInvite(token);return;}}catch(e){}finally{deviceScannerBusy=false;}}deviceScannerFrame=requestAnimationFrame(scanDeviceQrFrame);}
async function openDeviceScanner(){const modal=document.getElementById('modal-device-scanner'),status=document.getElementById('device-scanner-status'),video=document.getElementById('device-scanner-video');stopDeviceScanner();modal.classList.add('on');status.textContent='Запускаю камеру…';if(!navigator.mediaDevices?.getUserMedia){status.textContent='Это устройство не даёт Lumo доступ к камере. Введи ключ восстановления ниже в настройках синхронизации.';return;}if(!('BarcodeDetector' in window)&&typeof jsQR!=='function'){status.textContent='Модуль QR не загрузился. Проверь интернет и перезапусти Lumo.';return;}try{deviceQrDetector='BarcodeDetector' in window?new BarcodeDetector({formats:['qr_code']}):null;deviceScannerStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'},width:{ideal:1280},height:{ideal:1280}},audio:false});video.srcObject=deviceScannerStream;await video.play();status.textContent='Камера включена · наведи её на QR-код';deviceScannerFrame=requestAnimationFrame(scanDeviceQrFrame);}catch(e){stopDeviceScanner();status.textContent=e?.name==='NotAllowedError'?'Разреши доступ к камере для Lumo в настройках iPhone и попробуй снова.':'Не удалось включить камеру. Проверь, не используется ли она другим приложением.';}}
